...
### M2 · Billing lifecycle has no renewals and a single webhook event — status: FIXED (2026-09-13)
...
- Fix: implement period renewal (webhook-driven or cron auto-charge), add `payment.failed`/refund handling; until then every trial→paid conversion and every renewal needs a human.
